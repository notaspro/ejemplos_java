 ...
 * @author dwolverton
 */
public class HibernateMetadataUtil implements IMetadataUtil {
	...
	public boolean isId(Class<?> rootClass, String propertyPath) {
		...
		if (pos != -1) {
			IMetadata parentType = get(rootClass, propertyPath.substring(0, pos));
	...
	public IMetadata get(Class<?> entityClass) throws IllegalArgumentException {
	...
	public IMetadata get(Class<?> rootEntityClass, String propertyPath) throws IllegalArgumentException {
		try {
			IMetadata md = get(rootEntityClass);
			if (propertyPath == null || "".equals(propertyPath))
				return md;
