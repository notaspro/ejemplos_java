...
public abstract class BaseSearchProcessor {
	...
	protected IMetadataUtil metadataUtil;
	 ...
	 * the constructor.
	 */
	public IMetadataUtil getMetadataUtil() {
		return metadataUtil;
	}
	...
	protected String generateSimpleAllOrSome(SearchContext ctx, String property, Filter filter, String operation) {
		...
		Object value = SearchInternalUtil.convertIfNeeded(filter.getValue(), metadataUtil.get(ctx.rootClass, property)
				.getJavaClass());
		...
		if (isCollection) {
			...
			if (value instanceof Collection) {
				...
				for (Object item : (Collection) value) {
					val2[i++] = SearchInternalUtil.convertIfNeeded(item, expectedClass);
				}
			} else {
			...
			return SearchInternalUtil.convertIfNeeded(value, expectedClass);
		}
	}
	...
	public Filter getFilterFromExample(Object example, ExampleOptions options) {
		...
		IMetadata metadata = metadataUtil.get(example.getClass());
	...
	private void getFilterFromExampleRecursive(Object example, IMetadata metadata, ExampleOptions options,
		...
		for (String property : metadata.getProperties()) {
			...
			IMetadata pMetadata = metadata.getPropertyType(property);
